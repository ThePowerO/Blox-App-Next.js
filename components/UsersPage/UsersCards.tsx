...
export default function UsersCards({ users }: Props) {
  ...
  .sort((a, b) => {
    if (selectedFilter === "MostCombos") {
      ...
      return b.Combo.filter((combo) => combo.likes.length > 0).length - a.Combo.filter((combo) => combo.likes.length > 0).length;
    } else {
      return 0;
    }
  });
      ...
      <section className="flex flex-col gap-5">
        ...
        {searchValue === "" && selectedFilter === null && (
          <div className="flex border rounded-lg p-2 items-center gap-2">
            <Info size={20} />
            <p className="text-sm"> {t3("SearchUsersMessage")}
            </p>
          </div>
        )}
        {(searchValue !== "" || selectedFilter !== null) && filteredUsers.map((user) => (
          <div key={user.id} className={`w-full flex border rounded-lg p-2 gap-3`}>
